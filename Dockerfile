FROM --platform=$BUILDPLATFORM node:18.5.0-bullseye-slim AS cache-app
WORKDIR /app
...
FROM --platform=$BUILDPLATFORM node:18.5.0-bullseye-slim AS build-app
...
FROM --platform=$BUILDPLATFORM node:18.5.0-bullseye-slim AS cache-providers
WORKDIR /providers
...
FROM --platform=$BUILDPLATFORM node:18.5.0-bullseye-slim AS build-providers
WORKDIR /providers
